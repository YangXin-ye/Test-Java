...
```
个人信息按钮添加点击事件并且绑定 upInfo 方法
```js
upInfo(){
    this.$router.push('/upInfo')
}
```
methods 方法添加 upInfo 函数 
## 2、新增个人信息修改路由 router 文件夹下 index.js 文件
```js
 {
        path: '/upInfo',
        name: 'upInfo',
...
```
修改右上角头像为当前登陆用户的,修改 Header.vue 文件
```js
14行改为
...
```
![img.png](images/upapi.png)
## 4、新增 upInfo 文件夹到 pages 文件夹下，并且在 upInfo 文件夹下新增 upInfo.vue 文件
```vue
<template>
  <div>
...
```
# 二、后端
## 1、新增 UserController 接口文件到 controller 包下
```java
